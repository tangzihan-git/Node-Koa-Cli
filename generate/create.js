...
module.exports =  function(root,options={}) {
    ...
    const libTemplate = require(`../templates/${DataBaseLib}/lib_template`); //工具库模板文件
    ...
    models.forEach(async (f) => {
        ...
        // 将生成好的模型写入文件
        fs.writeFile(`${ModelRoot}/${modelName}`, commonModel(ctrlClass,generateModelMethods(ctrlClass)), (err) => {
            if (err) throw err;
    ...
    // 生成库文件
    generateLib(libRoot,libTemplate)
...
async function generateLib(libRoot, libTemplate){
    fs.writeFile(`${libRoot}/common.js`,libTemplate().makePageData , (err) => {
        if (err) throw err;
    });
    fs.writeFile(`${libRoot}/retjson.js`,libTemplate().retJson , (err) => {
        if (err) throw err;
    });
}
