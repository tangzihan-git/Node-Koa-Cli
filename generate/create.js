...
module.exports =  function(root,options={}, model='_model.js') {
    ...
    const models = glob.sync(`${ConfigModelRoot}/**/*${model}`); // 模型配置文件
    const allModels = glob.sync(`${ConfigModelRoot}/**/*_model.js`); // 所有模型配置文件
    ...
    // 生成路由文件控制器引用,并写入路由文件
    generateRouter(RouterRoot,routerTemplate(generateRouterMethods(allModels), options));
    // 用户是否配置了静态文件路由
